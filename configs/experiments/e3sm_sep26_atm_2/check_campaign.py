...
WANDB = {"project": "ACE2S-sep26v2-atm", "entity": "e3sm-aig"}
EMBED = {"embed_dim": 384}
VALIDATION_WINDOW = (1970, 1980)
...
def check(path: pathlib.Path) -> list[str]:
    ...
    for block in d.get("inference", []):
        ...
        want(
            name in INFERENCE_YEARS_BY_NAME,
            f"block {name!r} has no entry in INFERENCE_YEARS_BY_NAME",
        )
        want_years = INFERENCE_YEARS_BY_NAME.get(name)
        want(
            block["n_forward_steps"] == want_years * STEPS_PER_YEAR
        ...
        if block.get("weight") and want_years is not None:
