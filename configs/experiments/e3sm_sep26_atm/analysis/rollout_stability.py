        ...
        if os.path.exists(f):
            with xr.open_dataset(f) as ds:
                ...
                for v in HEAD:
                    ...
                    if k in ds:
                        b = ds[k].values
                        tm[v] = float(np.sqrt((w * b**2).sum() / (w * np.ones_like(b)).sum()))
        out[ep] = (s20, tm)
    return out
...
for ep in common:
    ...
    sp = v.max() - v.min(); seed_sp[ep] = (v.mean(), sp)
...
print(f"{'epoch':>6} " + " ".join(f"{n:>9}" for n in names) + f" {'range':>9} {'rho':>6} {'rho w/o E09':>12}   order")
...
for ep in ac:
    ...
    print(f"{ep:>6} " + " ".join(f"{x:>9.5f}" for x in v)
...
print(f"{'epoch':>6} {'range(all 5)':>13} {'range(no E09)':>14} {'seed spread':>12} {'ratio(no E09)':>14}")
for ep in ac:
    ...
    print(f"{ep:>6} {max(v)-min(v):>13.5f} {max(vs)-min(vs):>14.5f} {sp:>12.5f} {r:>14.1f}")
